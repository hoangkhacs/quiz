...
class MainWindow:
    ...
    def __init__(self, master):
        self.master = master
        self.master.geometry("1250x600+0+0")
        self.master.title("Câu hỏi trắc nghiệm")
        ...
        self.mainTitle = Label(f, text="Ứng dụng trả lời câu hỏi trắc nghiệm", fg="red", bg="azure", font=("Helvetica", 30, "bold italic")).place(
            x=400, y=200)
        self.regi = Button(f, text="Đăng ký", width=20, height=5, fg="royalblue4", bg="lavender",
                           font=("Helvetica", 10, "bold italic"), command=self.c_reg)
        self.login = Button(f, text="Đăng nhập", width=20, height=5, fg="royalblue4", bg="lavender",
...
class Register:
    ...
    def __init__(self, master):
        ...
        self.regNow = PhotoImage(
            file="D:\\workspace\\quiz\\Resources\\RegisterNow.png")
        imgRegNow = Label(f1, image=self.regNow, bg="azure")
        imgRegNow.place(x=550, y=5)

        self.mainTitle = Label(f1, text="Đăng ký tài khoản của bạn", bg="azure",
                               font=("Helvetica", 30, "bold italic underline")).place(x=50, y=10)
        self.name = Label(f1, text="Họ: ",
                          bg="azure", font=("Times New Roman", 10))
        self.lname = Label(f1, text="Tên: ",
                           bg="azure", font=("Times New Roman", 10))
        self.email = Label(f1, text="Email: ", bg="azure",
                           font=("Times New Roman", 10))
        self.uname = Label(f1, text="Tên đăng nhập:", bg="azure",
                           font=("Times New Roman", 10))
        self.pw = Label(f1, text="Mật khẩu: ",
                        bg="azure", font=("Times New Roman", 10))
        ...
        self.submit = Button(f1, text="Đăng ký", width=20, height=5, fg="royalblue4", bg="lavender",
                             font=("Helvetica", 10, "bold italic"), command=self.c_submit)
        self.cancel = Button(f1, text="Thoát", width=20, height=5, fg="royalblue4", bg="lavender",
                             font=("Helvetica", 10, "bold italic"), command=self.c_cancel)

        self.checkB = Checkbutton(f1, text='Hiện mật khẩu', bg="azure", fg="royalblue4",
    ...
    def check(self, l1):
        ...
        for i in range(5):
            ...
            if len(l1[i]) == 0:
                self.l = Label(
                    f1, text="Không thể để trống!", fg='red', bg="azure")
                self.l.place(x=400, y=ht)
            else:
                self.l = Label(
                    f1, text="Không thể để trống!", bg="azure", fg="azure")
                self.l.place(x=400, y=ht)
                f = f + 1
        if l1[2].find("@") == -1 and l1[2].find(".") == -1 and len(l1[2]) != 0:
            self.l = Label(
                f1, text="Giá trị email không hợp lệ!", bg="azure", fg="red")
            self.l.place(x=400, y=200)
            s = 1
        else:
            if (len(l1[2]) > 0):
                self.l = Label(
                    f1, text="Giá trị email không hợp lệ!", bg="azure", fg="azure")
                self.l.place(x=400, y=200)
        if len(l1[4]) < 8 and len(l1[4]) != 0:
            self.l = Label(
                f1, text="Mật khẩu phải có ít nhất 8 ký tự!", bg="azure", fg="red")
            self.l.place(x=400, y=300)
        else:
            if (len(l1[4]) > 0):
                self.l = Label(
                    f1, text="Mật khẩu phải có ít nhất 8 ký tự!", bg="azure", fg="azure")
                self.l.place(x=400, y=300)
    ...
    def c_submit(self):
        conn = MySQLdb.connect(
            host='localhost', database='world', user='root', password='leomi')
        cursor = conn.cursor()
        name = self.tname.get()
...
class Login:
    ...
    def __init__(self, master):
        ...
        self.l1 = Label(f2, text="Tên đăng nhập: ",
                        bg="azure", font=("Times New Roman", 20))
        self.e1 = Entry(f2, width=30)
        self.l2 = Label(f2, text="Mật khẩu: ",
                        bg="azure", font=("Times New Roman", 20))
        self.e2 = Entry(f2, width=30, show="*")
        self.b1 = Button(f2, text="Đăng nhập", width=15, height=3, fg="royalblue4", bg="lavender",
                         font=("Helvetica", 10, "bold italic"), command=self.clicked)
        self.b2 = Button(f2, text="Thoát", width=15, height=3, fg="royalblue4", bg="lavender",
                         font=("Helvetica", 10, "bold italic"), command=self.cancelLogin)

        self.var = IntVar()
        self.checkB = Checkbutton(f2, text='Hiện mật khẩu', bg="azure", fg="royalblue4",
                                  font=("Helvetica", 10, "bold italic"), variable=self.var, onvalue=1,
                                  offvalue=0, command=self.Showpasswd)
...
class Quiz:
    def __init__(self, master, u):
        ...
        self.Back = Button(f, text="Trở lại", width=15, height=3, fg="royalblue4", bg="snow2",
                           font=("Helvetica", 10, "bold italic"), command=self.back).place(x=100, y=325)
        self.Next = Button(f, text="Kế tiếp", width=15, height=3, fg="royalblue4", bg="snow2",
                           font=("Helvetica", 10, "bold italic"), command=self.next).place(x=250, y=325)
        self.submit = Button(f, text="Nộp bài", width=34, height=2, fg="ghost white", bg="DeepSkyBlue2",
                             font=("Helvetica", 10, "bold italic"), command=self.Submit).place(x=100, y=400)

        imgID = random.randint(1, 4)
        filestr = "D:\\workspace\\quiz\\Resources\\" + str(imgID) + ".png"
